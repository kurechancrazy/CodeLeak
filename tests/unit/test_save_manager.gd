...
func test_load_game_restores_high_score() -> void:
	_game_manager.high_score = 1200
	_save_manager.save_game()

	_game_manager.high_score = 0
	_save_manager.load_game()
	assert_eq(_game_manager.high_score, 1200, "ロード後に高スコアが復元されること")
...
func test_load_game_with_no_file_does_not_crash() -> void:
	_save_manager.load_game()
	pass  # クラッシュしなければOK（デフォルト値のまま）

func test_save_and_load_zero_score() -> void:
	_game_manager.high_score = 0
	_save_manager.save_game()
	_game_manager.high_score = 999
	_save_manager.load_game()
	assert_eq(_game_manager.high_score, 0, "スコア0が正しく保存・復元されること")
