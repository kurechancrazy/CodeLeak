...
- [x] 12.1 Godot エディタで `game_screen.tscn` を実行し、以下のシナリオを確認する
  - [x] レベル起動 → コードが行番号付きターミナル色で表示される（1.1.1〜1.1.4）
  - [x] マウスでコード行をホバー → ホバー色が変わる（9.2）
  - [x] コード行をクリック → 黄色ハイライト + 選択肢が右パネルに表示される（2.1.1, 3.1.1）
  - [x] キーボード `↑/↓` でハイライト移動 → `Enter` で選択確定（2.2.1, 2.2.2）
  - [x] 正解選択 → 緑フラッシュ + `>> PATCHED` 表示（6.1.1）
  - [x] 不正解選択 → 赤フラッシュ + `>> ACCESS DENIED` + 即リトライ（6.2.1, 3.2.3）
  - [x] `[HINT]` ボタン押下 → 対象行にマーカー表示 + ヒント残数減少（4.2）
  - [x] ヒント残数 0 → ヒントボタンが無効化される（4.3）
  - [x] 全問正解 → `[SYSTEM SECURED]` タイプライター演出 → リザルト遷移（6.3.1, 6.3.2）
  - [x] `Esc` でポーズ → タイマー停止 + ポーズメニュー表示（7.1〜7.3）
  - [x] `[RESTART LEVEL]` → 状態リセット（7.5）
  - [x] セーブファイルにスコアが記録されていること（5.3）
